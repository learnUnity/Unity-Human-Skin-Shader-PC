...
struct v2f_SubSurface{
  float4 vertex : SV_POSITION;
  float2 uv : TEXCOORD0;
};
// vertex shader
inline v2f_SubSurface vert_surf (appdata_base v) {
	v2f_SubSurface o;
	o.vertex = UnityObjectToClipPos(v.vertex);
  o.uv = TRANSFORM_TEX(v.texcoord, _BlurMap);
	return o;
}
...
[UNITY_domain("tri")]
inline v2f_SubSurface ds_surf (UnityTessellationFactors tessFactors, const OutputPatch<InternalTessInterp_appdata_base,3> vi, float3 bary : SV_DomainLocation) {
  appdata_base v;
  ...
  disp_shadow(v);
  v2f_SubSurface o = vert_surf (v);
  return o;
}

inline float4 frag_surf (v2f_SubSurface IN) : SV_Target {
 	return float4(tex2D(_BlurMap, IN.uv).r * _BlurIntensity * 0.3333333333,1,1,1);
}
ENDCG
}
}
